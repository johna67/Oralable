...
private actor PersistenceWorker {
    ...
    func writeAccelerometerFrame(_ frame: AccelerometerFrame) {
        self.persistence.writeAccelerometerFrame(frame)
    }
    
    func writePPGFrame(_ frame: PPGFrame) {
        self.persistence.writePPGFrame(frame)
    }
}
...
@MainActor
@Observable final class MeasurementService {
...
//    private var muscleActivitySummarizer = MeasurementSummarizer(range: 100000.0..<150000.0)
    ...
    private let muscleActivityMagnitudeRange: Range<Double> = 100000.0..<150000.0
...
//                movementSummary = movementSummarizer.addMeasurement(measurement)
    ...
    private func processMeasurement<T: MeasurementConvertible>(frame: T, range: Range<Double>, currentMinute: inout (date: Date, count: Int)?, summary: inout [SummaryData]) {
    ...
    private func convert<T: MeasurementConvertible>(_ frame: T) -> MeasurementData {
