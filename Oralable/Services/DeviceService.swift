...
class TGMService: DeviceService {
    ...
    lazy var batteryVoltage: AsyncStream<Int> = {
    ...
    lazy var ppg: AsyncStream<PPGFrame> = {
        AsyncStream(bufferingPolicy: .unbounded) { continuation in
            self.ppgContinuation = continuation
        }
    }()
    
    lazy var accelerometer: AsyncStream<AccelerometerFrame> = {
        AsyncStream(bufferingPolicy: .unbounded) { continuation in
            self.accelerometerContinuation = continuation
        }
    }()
    
    lazy var temperature: AsyncStream<Double> = {
        AsyncStream(bufferingPolicy: .unbounded) { continuation in
    ...
    func start() async throws {
        ...
        try parseBatteryData(try await readData(for: batteryCharacteristic))
